package Days is
   procedure Run_Day_1( Input_File: String );

   procedure Run_Day_2( Input_File: String );

   procedure Run_Day_3( Input_File: String );

   procedure Run_Day_4( Input_File: String );

   procedure Run_Day_5( Input_File: String );

   procedure Run_Day_6( Input_File: String );

   procedure Run_Day_8( Input_File: String );

   procedure Run_Day_9( Input_File: String );

   procedure Run_Day_10( Input_File: String );

   procedure Run_Day_11( Input_File: String );

   procedure Run_Day_12( Input_File: String );

   procedure Run_Day_13( Input_File: String );

   procedure Run_Day_14( Input_File: String );

   procedure Run_Day_15( Input_File: String );
end Days;
